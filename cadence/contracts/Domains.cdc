...
pub contract   Domains: NonFungibleToken{
    ...
    init() {
        ...
        self.DomainsStoragePath = StoragePath(identifier: "flowNameServiceDomains")?? panic("Could not set the storage path")
        self.DomainsPrivatePath = PrivatePath(identifier: "flownameServiceDomains")?? panic("Could not set the private path")
        self.DomainsPublicPath = PublicPath(identifier: "flowNameServiceDomains")?? panic("Could not set the public path")



        self.RegistrarStoragePath = StoragePath(identifier: "flowNameServiceRegistrar") ?? panic("Could not set the storage path")
        self.RegistrarPrivatePath = PrivatePath(identifier: "flowNameServiceRegistrar")?? panic("Could not set the private path" )
        self.RegistrarPublicPath = PublicPath(identifier: "flowNameServiceRegistrar") ?? panic("Could not seyt the piblic path")

        self.account.save(<- self.createEmptyCollection(), to: Domains.DomainsStoragePath)

        self.account.link<&Domains.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, Domains.CollectionPublic}>(self.DomainsPublicPath, target: DomainsStoragePath)

        self.account.link<&Domains.Collection>(self.DomainsPrivatePath, target: self.DomainsStoragePath)

        let collectionCapability = self.account.getCapability<&Domains.Collections>(self.DomainsPrivatePath)
    ...
    pub resource Registrar: RegistrarPublic, RegistrarPrivate {
        ...
        pub fun renewDomain(domain: &Domains.NFT, duration: UFix64, feeTokens: @FungibleToken.Vault) {
          ...
          self.domainsCollection.borrow()!.mintDomain(name: name, nameHash: nameHash, expiresAt: expirationTime, receiver: receiver)
            ...
            // Return balance of rentVault
            pub fun getVaultBalance(): UFix64 {
                return self.rentVault.balance
            }

            // Point the rentVault to another vault
            pub fun updateRentVault(vault: @FungibleToken.Vault) {
                pre {
                    self.rentVault.balance == 0.0 : "Withdraw balance from the old vault before updating"
                }
